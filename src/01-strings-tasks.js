...
function getCardId(card) {
  ...
  const cardValue = card.slice(0, card.length - 1);

  const suitWight = DESK_WIEGHT[suit];
  const cardValueWight = cardValue in DESK_WIEGHT ? DESK_WIEGHT[cardValue] : +cardValue;

  const cardIndex = cardValueWight - 1 + suitWight * 13;
